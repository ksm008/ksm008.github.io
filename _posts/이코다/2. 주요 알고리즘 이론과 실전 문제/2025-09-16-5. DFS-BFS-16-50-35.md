...
**자료구조**는 데이터를 표현하고 관리하고 처리하기 위한 구조이다. 그 중 스택과 큐가 기초 개념. 푸시와 팝을 사용하여 데이터를 각각 삽입/삭제를 한다.
...
큐는 입구와 출구가 하나씩이지만 양쪽 모두 삽입과 삭제가 가능한 덱(Deque)도 존재한다. 스택과 큐의 장점만을 채택한 자료구조인데, 데이터를 넣고 빼는 속도가 리스트 자료에 비해 효율적이다. 
...
### BFS
Breadth First Search, BFS는 너비 우선 탐색이라는 의미를 가진다. 가까운 노트부터 탐색하는 알고리즘이다. DFS는 스택을 사용했던 것과 다르게 BFS는 큐를 사용한다.

탐색 시작 노드에 큐를 삽입하고 방문 처리를 한 후, 다시 노드를 꺼내 인접 노드 중 방문하지 않은 노드를 모두 큐에 삽입하고 방문 처리를 한다. 이 과정은 큐가 전부 빌 때까지 반복된다.

참고로 DFS든 BFS든 각 노드의 리스트는 오름차순 정렬이 되어있어야 한다. 그렇지 않으면 방문하게 되는 노드가 달라질 것이다.
...
아까 만들었던 dfs와 합쳐 백준의 1260번 [DFS와 BFS](https://www.acmicpc.net/problem/1260) 문제를 풀었다.
...
public class Main {  
    ...
    public static void main(String[] args) throws IOException {  
        ...
        iceTemplate =  new int[N][M];  
  
        for (int i = 0; i < N; i++) {  
            String str = br.readLine();  
            for (int j = 0; j < M; j++) {  
                iceTemplate[i][j] = str.charAt(j) - '0';  
            }  
        }  
