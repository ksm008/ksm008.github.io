...
tags:
  ...
  - 문자열
---
문자열을 계속해서 뒤집고 삭제하는건 시간 낭비이다. 때문에 플래그를 알려주는 불린 값을 하나 사용하고, 앞 뒤 양쪽으로 삭제가 가능한 데큐를 사용하면 된다.
에러가 발생하는 때는 숫자가 없을 때 삭제 연산을 할 때이다. 만약 데큐가 비어있는데 삭제 연산을 하게 될 경우, 이 때만 에러를 출력해주면 된다. 
