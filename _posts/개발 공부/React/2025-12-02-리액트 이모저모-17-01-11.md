...
만약 useState에 담긴 객체, 배열을 변경한다면 자바처럼 내용을 직접 수정하면 안된다. 반드시 새로운 객체나 배열을 만들어서 세터 함수에 넣어줘야 한다. 그렇지 않으면 리액트가 데이터가 바뀌었는지 감지를 하지 못해서 화면을 새로 그리지 않는다.

```jsx
// ❌ 객체 직접 수정
const [user, setUser] = useState({ name: 'Kim' });
const changeName = () => {
  user.name = 'Lee'; // 😱 객체 내부만 바꿈. 주소는 그대로!
  setUser(user);    // 리액트가 변화를 감지 못함 (렌더링 안 됨)
};

// ❌ 배열 직접 수정 (Array List의 add, remove와 다름)
const [list, setList] = useState([1, 2, 3]);
const addItem = () => {
  list.push(4); // 😱 배열 내부만 바꿈. 주소는 그대로!
  setList(list); // 리액트가 변화를 감지 못함 (렌더링 안 됨)
};
```

때문에 반드시 펄침 연산자를 사용하여 기존 내용을 복사하고 원하는 부분만 덮어씌워 새로운 객체나 배열을 만들어 전달해야한다.

- 펄침 연산자: `...` 점 3개를 찍는 문법이다. 객체나 배열을 복사하는데 사용한다.

```jsx
// ✅ 객체 불변성 관리 (기존 내용을 복사하고 name만 덮어쓰기)
const [user, setUser] = useState({ name: 'Kim', age: 20 });
const changeName = () => {
  // 기존 user 객체를 ...user로 복사하고, name: 'Lee'를 덮어씀
  setUser({ ...user, name: 'Lee' }); 
};

// ✅ 배열 불변성 관리 (기존 배열을 복사하고 4를 추가)
const [list, setList] = useState([1, 2, 3]);
const addItem = () => {
  // ...list로 기존 배열을 복사한 뒤, 뒤에 4를 추가해서 새 배열을 만듦
  setList([...list, 4]); 
};
```

### 조건부 렌더링
조건식을 사용하여 렌더링을 하는 조건부 렌더링에도 useState가 쓰인다.

#### 1. 삼항 연산자를 사용하는 방법

```jsx
// 상태: isPending이 true면 로딩 중, false면 완료
...
#### 2. && 연산자 사용 방법 (자바의 &&와 다름)

```jsx
// 상태: errorMessage가 문자열(true)이면 표시, null/undefined(false)면 표시 안 함
...
return (
  <div>
    ...
    {/* && 연산자는 앞의 조건이 참(true)일 때만 뒤의 JSX를 반환함 */}
    {errorMessage && (
      <p style={{ color: 'red' }}>에러 발생: {errorMessage}</p>
    )}
  </div>
);
```
...
참고로 함수 앞에 async가 붙어있다면 return이 없어도 Promise 객체가 반환된다. (`Promise<undefined>`)