...
보통 실무에서는 아예 따로 열거 파일을 만들어 놓는다. 여러 곳에서 쓰기 편하고, 코드를 찾기 쉽게 하기 위함이다.
...
- 타입 안정성 : Direction 타입 변수에는 정의된 값만 할당할 수 있다. int나 String을 쓸 때 처럼 다른 타입의 엉뚱한 값이 들어오는걸 막아준다.
- 컴파일 타임 상수 : 각 enum 값은 컴파일 시점에 생성되는 싱글톤 객체로 취급되어 Direction.NORTH는 프로그램 전체에서 딱 하나의 인스턴스만 존재한다. 때문에 enum은 기본적으로 static 성질을 띈다.



### 필드, 생성자, 메소드

enum은 필드와 메소드를 가질 수 있다. 때문에 일반 클래스처럼 동작한다.

#### 필드와 생성자 추가 
상수 각각이 고유한 데이터를 가질 수 있게 해준다.
...
Enum  내부에서 해당 상수에 특화된 로직(메소드)을 정의하거나 특정 메소드를 재정의해서 상수별로 다른 행동을 하게 만들 수 있다.
...
int result = Operator.PLUS.apply(10, 5); 
```

상수마다 다른 행위를 구현하는 방법을 상수별 메소드 구현이라고 한다. switch 문으로 길게 조건을 나열할 필요 없이 객체 지향적으로 깔끔하게 로직을 분리할 수 있게 해준다.

#### 기본 제공 메소드

java.lang.Enum을 상속받기 때문에 모든 enum 타입은 아래의 메소드들을 사용할 수 있다.

| **메서드**                | **용도**                        | **예시 (OrderStatus.DELIVERED)**   |
| ---------------------- | ----------------------------- | -------------------------------- |
| `name()`               | 상수 이름을 **문자열**로 반환            | `"DELIVERED"`                    |
| `ordinal()`            | Enum에 정의된 **순서(0부터 시작)**를 반환  | `2`                              |
| `valueOf(String name)` | 문자열 이름으로 **Enum 인스턴스**를 찾아 반환 | `OrderStatus.valueOf("SHIPPED")` |
| `values()`             | Enum에 정의된 **모든 상수 배열**을 반환    | `[PENDING, SHIPPED, DELIVERED]`  |
